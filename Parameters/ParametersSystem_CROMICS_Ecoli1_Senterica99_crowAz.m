...
tsim=48*3600*1000;   %Simulation time [ms].
...
Ecoli=1;            %Porcentage of the total community population that belongs to E. coli.


%Name used to save the dataFile (include the path, it is suggested to use the folder "Data" ):  
DataFile=strcat('./Data/dataForCROMICS_Ecoli',num2str(Ecoli),'_Senterica',num2str(100-Ecoli),'_crow',num2str(Vocc),'.mat')
%Name that will be used to save the results (include the path, it is suggested to use the folder "Results" ):   
ResultsFile=strcat('./Results/resultsCROMICS_Ecoli',num2str(Ecoli),'_Senterica',num2str(100-Ecoli),'_crow',num2str(Vocc));
...
bacterialspots=10;%65; %Number of bacterial spots [dimensionless].
massB=(3e-7);%/bacterialspots; %Total bacterial mass (all species) per bacterialspot [gDW].
nN=20;
