function [frac,VavLBM]=PavailableSpace_EPS(v_eps,fepsCell1,v_met,CellSizeVar,Vcell_cte,vol_lbm,v_sp,fwater,IDpos,Ny_lbm,Nx_lbm,Nz_lbm,num_met,frac_medium,Ncells,frac)
...
%Compute the space fraction occupied by EPS associate to the cell biomass
ff_eps=IDpos(1:Ncells,3).*(sum(fepsCell1(IDpos(1:Ncells,2),:).*v_eps,2))/vol_lbm;
frac1_eps(1:max(IDpos(1:Ncells,4)))=accumarray(IDpos(1:Ncells,4),ff_eps);
frac_eps(:)=repmat(frac1_eps,num_met,1);


%Compute the space fraction available for the metabolites
frac(:)=frac_medium(:)-frac(:)-frac_eps(:); %Matrix with the probability to find available space in the LBM-grid [dimensionless].
