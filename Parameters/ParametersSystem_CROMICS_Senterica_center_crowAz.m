...
Vocc=0.2;  %Volume fraction occupied by cells
Vmax_sp=1; %For active cells
% Vmax_sp=0; %For inactive cells
...
tsim=.1*3*3600*1000;   %Simulation time [ms].
At=312.5000*2;%125000;           %Time step [ms]. 
nAt=30;              %The simulation results will be record every nAt*At [ms].  
...
Ecoli=99;            %Porcentage of the total community population that belongs to E. coli.


if Vmax_sp==0 %For inactive cells
    %Name used to save the dataFile (include the path, it is suggested to use the folder "Data" ):  
    nameDataFile=strcat('dataForCROMICS_Senterica_center_Vocc',num2str(Vocc),'_Vmax0_meth.mat')
    DataFile=strcat('./Data/',nameDataFile)
    %Name that will be used to save the results (include the path, it is suggested to use the folder "Results" ): 
    ResultsFile=strcat('./Results/resultsCROMICS_Senterica_center_Vocc',num2str(Vocc),'_Vmax0_meth');

else %For active cells
    %Name used to save the dataFile (include the path, it is suggested to use the folder "Data" ):  
    nameDataFile=strcat('dataForCROMICS_Senterica_center_Vocc',num2str(Vocc),'_meth.mat')
    DataFile=strcat('./Data/',nameDataFile)
    %Name that will be used to save the results (include the path, it is suggested to use the folder "Results" ): 
    ResultsFile=strcat('./Results/resultsCROMICS_Senterica_center_Vocc',num2str(Vocc),'_meth');
end
...
Cmet=[2.92,0.21,0,10,0];     %Initial concentration of the metabolites [mM]. [lactose, O2, methionine, acetate]. The concentration will be changed to [mmol/mm^3] below.
...
boundaryLBM.Z0={'D','D','B','D','B'};
CmetDb.Z0=[2.92,0.21,0,10,0];
...
bacterialspots=1; %Number of bacterial spots [dimensionless].
...
my=ceil(Ny_ca/2)+1;
rhoCA_sp(:,:,1,1)=mu;
rhoCA_sp(my,mx,1,2)=rhoCA_sp(my,mx,1,1);
rhoCA_sp(my,mx,1,1)=0;
medium(:,:,:)=2;
...
if strfind(subpop_analysis,'YES')==1 %Track subpopulations  
    ...
    sp=2;
    ff=find(rhoCA_sp(:,:,:,sp));
    rN2=ones(length(ff),1); %rN1=1 for mutants 
