...
DataFile=strcat('./Data/dataForCROMICS_Ecoli_30eps.mat')
%Name that will be used to save the results (include the path, it is suggested to use the folder "Results" ):  
ResultsFile=strcat('./Results/resultsCROMICS_Ecoli_30eps');
...
metaB=[27;12];           %Number of cells in one metabacteria, equivalent to Ax_ca/maximumCellRadius, maximumCellRadius=0.0019mm. 
...
NNlist={'NN15x15_Ecoli_glc_pTFA';'NN15x15_Ecoli_glc_pTFA_30eps'}; %List of the NN files to be used for the metabolic flux computation. 
...
%EPS:
fepsCell1=[0;0.3/.7]; %Ratio g_eps/g_cell. 
MMeps=[250000000]; %Average molecular weight of EPS molecules [g mol^-1] 
v_eps=v_sp(2)*3;
Reps1=(MMeps*v_eps/(4*pi*AvN/3)).^(1/3); %[mm].
fepsCell=(fepsCell1./MMeps).*AvN;
