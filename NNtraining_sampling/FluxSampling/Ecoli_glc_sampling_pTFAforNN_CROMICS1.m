...
load Ecoli_glc_ac_o2_TFA.mat
model.var_ub(ismember(model.varNames,'R_DM_ac_e'))=0;
model.var_lb(ismember(model.varNames,'R_DM_ac_e'))=0; 
model.var_ub(ismember(model.varNames,'F_DM_ac_e'))=1000;
model.var_ub(ismember(model.varNames,'R_DM_o2_e'))=0;
model.var_lb(ismember(model.varNames,'R_DM_o2_e'))=0; 
model.var_ub(ismember(model.varNames,'R_DM_glc_e'))=10;
model.var_lb(ismember(model.varNames,'R_DM_glc_e'))=0; 
...
rxnNames={'DM_glc_e';'DM_o2_e';'DM_ac_e';'Ec_biomass_iJO1366_WT_53p95M'};
rxnInterest=3;

%Range of fluxes in which samples will be taken. 
%No sample is taken if Vmin(rxn) = Vmax(rxn) = 0 for reaction rxn
Vmin=[0;0;0;0];  %[mmol gDW^-1 h^-1]
Vmax=[10;10;0;0];%[mmol gDW^-1 h^-1] %v3

%Number of samples to generate
nsamples=30000;

%Name of the file to save the input and output data
nameFile='TestPoints_Ecoli_glc_o2_ac_pTFA_30K.mat'
...
%uniqueCsource={};
uniqueCsource={'DM_glc_e'};%;'DM_ac_e'};
...
% Output variable
target_solTable = zeros(nsamples,rxn+rxn_minmax);
