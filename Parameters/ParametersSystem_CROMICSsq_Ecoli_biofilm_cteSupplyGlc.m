...
tsim=6*60*1000;  %Simulation time [ms].
At=30*1000;        %Time step [ms]. 
nAt=2;               %The simulation results will be record every nAt*At [ms].  
...
glc=2.25; %Glucose concentration [mM] that will be supply to the system throught the top boundary. 

%Name used to save the dataFile (include the path, it is suggested to use the folder "Data" ):  
DataFile=strcat('./Data/dataForCROMICSsq_Ecoli_biofilm_cteSupply',num2str(glc),'.mat')
%Name that will be used to save the results (include the path, it is suggested to use the folder "Results" ):   
ResultsFile=strcat('./Results/resultsCROMICSsq_Ecoli_biofilm_cteSupply',num2str(glc));


%Parameters CA
distanceX=0.1901/5;   %[mm].
distanceY=0.1901*3;%[mm].
distanceZ=.0019;    %[mm].
...
vshrinkage=0.016*0;       %Cell shrinkage rate [h^-1].
...
Dmet=[6.7*10^-7;2*10^-6;1.21*10^-6];%Diffusion coefficient of metabolites [mm^2 ms^-1]. [lactose, O2, methionine, acetate, galactose]
...
Cmet=[0,0,0];%Initial concentration of the metabolites [mM]. [glucose, O2, acetate]. The concentration will be changed to [mmol/mm^3] below.
...
boundaryLBM.Y0={'D','D','B'};               
CmetDb.Y0=[glc,0.21,0];         %[mM]. The concentration will be changed to [mmol/mm^3] below.
boundaryLBM.Yn={'D','D','B'};
...
%Matrix-CA of the density distribution for cell species [g].
%The system is filled with cells.
rhoCA_sp=normrnd(mu,sigma,[Ny_ca,Nx_ca,Nz_ca,num_sp]); %Biomass from a normal distribution

% save ('rhoCA_sp_biofilm_cteSupply.mat','rhoCA_sp')
load rhoCA_sp_biofilm_cteSupply.mat
